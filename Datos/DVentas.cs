        ...
        public List<DetallesVentas> ObtenerDetallesVenta(int idventa)
        {
            ...
            using (SqlConnection oconexion = new SqlConnection(Conexion.cadena))
            {
                try
                {
                    oconexion.Open();
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("select p.codigo + ' ' + p.nombre + ' ' + p.descripcion + ' ' + modelo + ' ' + marca as productos, p.descuento, dv.precioventa, dv.cantidad, dv.subtotal from detalle_venta dv");
